import { useState, useEffect } from "react";

export default function CautionsCompareModal({ toolA, toolB, onClose }) {
    const [dataA, setDataA] = useState(null);
    const [dataB, setDataB] = useState(null);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);

    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            setError(null);
            try {
                const [responseA, responseB] = await Promise.all([
                    fetch(`/api/tool/${toolA.Slug}/cautions`),
                    fetch(`/api/tool/${toolB.Slug}/cautions`),
                ]);

                if (!responseA.ok || !responseB.ok) {
                    throw new Error(`HTTP error!`);
                }

                const resultA = await responseA.json();
                const resultB = await responseB.json();

                setDataA(resultA);
                setDataB(resultB);
            } catch (e) {
                console.error("Failed to fetch cautions:", e);
                setError("Failed to load cautions. Please try again.");
            } finally {
                setIsLoading(false);
            }
        }

        fetchData();
    }, [toolA.Slug, toolB.Slug]);

    return (
        <div
            className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
            onClick={onClose}
        >
            <div
                className="bg-cardDark p-6 rounded-lg shadow-lg max-w-4xl w-full relative"
                onClick={(e) => e.stopPropagation()}
            >
                <button
                    onClick={onClose}
                    className="absolute top-2 right-2 text-gray-100 hover:text-gray-300 text-xl"
                >
                    &times;
                </button>
                <h3 className="text-xl font-bold mb-4 text-center">
                    Caution Comparison
                </h3>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <h4 className="text-lg font-semibold mb-2">{toolA.Name}</h4>
                        {isLoading && <p>Loading...</p>}
                        {error && <p className="text-red-400">{error}</p>}
                        {dataA && dataA.length > 0 ? (
                            <ul className="list-disc ml-6 space-y-2">
                                {dataA.map((caution, index) => (
                                    <li key={index}>{caution.Caution}</li>
                                ))}
                            </ul>
                        ) : (
                            !isLoading && !error && <p>No cautions found.</p>
                        )}
                    </div>
                    <div>
                        <h4 className="text-lg font-semibold mb-2">{toolB.Name}</h4>
                        {isLoading && <p>Loading...</p>}
                        {error && <p className="text-red-400">{error}</p>}
                        {dataB && dataB.length > 0 ? (
                            <ul className="list-disc ml-6 space-y-2">
                                {dataB.map((caution, index) => (
                                    <li key={index}>{caution.Caution}</li>
                                ))}
                            </ul>
                        ) : (
                            !isLoading && !error && <p>No cautions found.</p>
                        )}
                    </div>
                </div>
            </div>
        </div>
    );
}