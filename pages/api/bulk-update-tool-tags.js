...
async function handler(req, res) {
    ...
    try {
        ...
        const [allTools, tags] = await Promise.all([
            getAllTools(),
            getAllTags(),
        ]);
        ...
        for (const tool of allTools) {
            try {
                const generatedTags = await  generateToolTags(tool, availableTags, model);
        ...
        sendEvent('status', { message: "The bulk update for tool tags is complete. " });
        res.end();
        return;
    } catch (error) {
