import { getToolBySlug } from "@/lib/airtable/tools";
import { getUseCasesByTool } from "@/lib/airtable/use-cases";

import UseCaseCompareCard from "@/components/UseCaseCompareCard";
import MetaProps from "@/components/MetaProps";

export async function getStaticPaths() {
    try {
        // We don't pre-render any comparison paths at build time.
        // They will be generated on-demand with fallback: "blocking".
        return { paths: [], fallback: "blocking" };
    } catch (error) {
        console.error("[getStaticPaths - Compare] Error:", error);
        return { paths: [], fallback: "blocking" };
    }
}

export async function getStaticProps({ params }) {
    const { slugA, slugB } = params;

    try {
        const [toolA, toolB, useCasesA, useCasesB] = await Promise.all([
            getToolBySlug(slugA),
            getToolBySlug(slugB),
            getUseCasesByTool(slugA),
            getUseCasesByTool(slugB),
        ]);

        if (!toolA || !toolB) {
            return { notFound: true };
        }

        return {
            props: { toolA, toolB, useCasesA, useCasesB },
            revalidate: 300, // Revalidate every 5 minutes
        };
    } catch (error) {
        console.error(
            `[getStaticProps - Compare] Error for slugs ${slugA}, ${slugB}:`,
            error,
        );
        return { notFound: true };
    }
}

export default function ComparePage({ toolA, toolB, useCasesA, useCasesB }) {
    if (
        !toolA ||
        !toolB ||
        !toolA.Name ||
        !toolB.Name ||
        !toolA.Slug ||
        !toolB.Slug
    ) {
        return (
            <div className="max-w-2xl mx-auto py-12 text-center text-gray-300">
                <h2 className="text-xl font-bold mb-2">
                    Comparison Not Available
                </h2>
                <p>
                    Sorry, one or both tools could not be loaded. Please try
                    again later.
                </p>
            </div>
        );
    }

    return (
        <>
            <MetaProps
                title={`Compare ${toolA.Name} Use Cases Against ${toolB.Name} Use Cases`}
                description={`Compare ${toolA.Name} and ${toolB.Name} across use cases.`}
                url={`${
                    process.env.NEXT_PUBLIC_BASE_URL ||
                    "https://aitoolpouch.com"
                }/compare/${toolA.Slug}/vs/${toolB.Slug}/use-cases`}
            />
            <div className="max-w-7xl mx-auto">
                <div className="h-full flex items-center justify-between mb-6">
                    <h1 className="text-2xl font-bold">
                        Comparing {toolA.Name} Use Cases -to- {toolB.Name} Use
                        Cases
                    </h1>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <UseCaseCompareCard
                        key={toolA.Slug}
                        tool={toolA}
                        useCases={useCasesA}
                    />
                    <UseCaseCompareCard
                        key={toolB.Slug}
                        tool={toolB}
                        useCases={useCasesB}
                    />
                </div>
            </div>
        </>
    );
}
