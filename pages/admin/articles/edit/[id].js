...
export async function getServerSideProps({ req, res, params }) {
    ...
    if (req.method === "POST") {
        try {
            ...
            return {
                props: { error: `Failed to update article: ${error.message}` },
            };
        }
    }
    ...
    try {
        ...
        return {
            props: { error: `Failed to fetch article: ${error.message}` },
        };
    }
}
...
export default function EditArticlePage({ article, error }) {
    ...
    useEffect(() => {
        if (article) {
            // You might want to set initial values for AI generation based on existing article
            // For now, leaving them as default or empty
        }
    }, [article]);
    ...
    return (
        <div className="w-full md:w-[75%] mx-auto">
            ...
            <div className="bg-cardDark p-6 rounded-lg shadow-md mb-6">
                ...
                {clientError && (
                    <p className="text-red-500 mt-2">{clientError}</p>
                )}
            </div>
