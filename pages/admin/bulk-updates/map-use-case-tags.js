...
export default function UpdateUseCaseTagsPage() {
    ...
    const { statusLog, error, isLoading, startStream } = usePersistentSSE({
        url: `/api/admin/bulk-updates/map-use-case-tags?model=${model}`,
    });
    ...
    return (
        <div className="flex items-center justify-center py-12">
            <div className="w-full max-w-2xl p-8 space-y-6 bg-cardDark rounded-lg border border-gray-600">
                ...
                <h1 className="text-2xl font-bold mb-2">Map Use Case Tags</h1>
                <p className="mb-4">
                    This process uses AI to Analyze use case tags and map them
                    to tools. It will iterate over every tool, compare the tool
                    to listed use case tags, and select the most relevant tags
                    to assign to the tool. The existing tags for each tool will
                    be overwritten.
                </p>
                ...
                {(statusLog.length > 0 || error) && (
                    <div
                        id="status-container"
                        className="mt-6 p-4 rounded-md bg-gray-800 border border-gray-600 max-h-96 overflow-y-auto"
                    >
