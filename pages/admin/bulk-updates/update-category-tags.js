...
export default function UpdateCategoryTags() {
  ...
  const handleStartUpdate = () => {
    ...
    const eventSource = new EventSource(
      `/api/bulk-update-category-tags?model=${model}`,
    );
    ...
    eventSource.addEventListener("fatal_error", (event) => {
      ...
      setStatusLog((prevLog) => [
        ...prevLog,
        data.message || "A fatal error occurred.",
      ]);
      setIsLoading(false);
      eventSource.close();
    });
  ...
  return (
    <div className="flex items-center justify-center py-12">
      <div className="w-full max-w-2xl p-8 space-y-6 bg-cardDark rounded-lg border border-gray-600">
        ...
        <h1 className="text-2xl font-bold mb-2">Bulk Update Category Tags</h1>
        <p className="mb-4">
          This process uses AI to analyze preferences. It will iterate over
          every preference, analyze its purpose, and select 2-10 of the most
          relevant tags from the master list to assign to it. The existing tags
          for each preference will be overwritten.
        </p>
        <p className="mb-6 text-yellow-400">
          Warning: This action is irreversible and may take a significant amount
          of time to complete depending on the number of preferences.
        </p>
