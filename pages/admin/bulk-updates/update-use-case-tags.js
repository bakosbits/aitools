...
export default function UpdateUseCaseTagsPage() {
  ...
  const handleStartUpdate = () => {
    ...
    const eventSource = new EventSource(
      `/api/bulk-update-use-case-tags?model=${model}`,
    );
  ...
  return (
    <div className="flex items-center justify-center py-12">
      <div className="w-full max-w-2xl p-8 space-y-6 bg-cardDark rounded-lg border border-gray-600">
        ...
        <p className="mb-4">
          This process uses AI to Analyze use cases. It will iterate over every
          use case, analyze its purpose, and select 3-10 of the most relevant
          tags from the master list to assign to it. The existing tags for each
          use case will be overwritten.
        </p>
        <p className="mb-6 text-yellow-400">
          Warning: This action is irreversible and may take a significant amount
          of time to complete depending on the number of use cases.
        </p>
