  ...
  const filteredTools = useMemo(() => {
    ...
    return sortedTools.filter((tool) => {
      ...
      return searchableFields.some(
        (field) => typeof field === "string" && field.match(searchRegex),
      );
    });
  }, [query, sortedTools]);
    ...
    <>
      <div className="w-full md:w-[75%] mx-auto">
        <div>
          <Link
            href="/admin"
            className="text-slate-300 hover:text-gray-100 mb-6 inline-block"
          >
                ...
                paginatedTools.map((tool) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-sm">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-semibold ${tool.Active ? "bg-green-500 text-slate-100" : "bg-yellow-200 text-yellow-800"}`}
                      >
                    ...
                    <td className="px-5 py-4 text-sm text-right">
                      <Link
                        ...
                        className="text-slate-300 hover:text-gray-100 mr-4"
                      >
                        Edit
                      </Link>
                      <form
