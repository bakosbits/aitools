...
export async function getServerSideProps({ req, res }) {
  if (req.method === "POST") {
    try {
      ...
      if (toolData.Pricing) {
        toolData.Pricing = Array.isArray(toolData.Pricing)
          ? toolData.Pricing
          : [toolData.Pricing];
      } else {
        toolData.Pricing = [];
      ...
      if (toolData.Tags && typeof toolData.Tags === "string") {
        toolData.Tags = toolData.Tags.split(",")
          .map((tag) => tag.trim())
          .filter(Boolean);
      ...
      if (toolData.Categories) {
        toolData.Categories = Array.isArray(toolData.Categories)
          ? toolData.Categories
          : [toolData.Categories];
      } else {
        toolData.Categories = [];
      ...
      if (toolData.Articles) {
        toolData.Articles = Array.isArray(toolData.Articles)
          ? toolData.Articles
          : [toolData.Articles];
      } else {
        toolData.Articles = [];
