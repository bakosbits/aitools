...
const validateTagsForEntity = async (
  ...
  try {
    ...
    if (validTagsNames.length !== result.Tags?.length) {
      console.warn(
        `Some invalid tags were filtered out for ${entityType}. Original:`,
        result.Tags,
        "Valid:",
        validTagsNames,
      );
...
export async function generateUseCaseTags(
  useCase,
  availableTags,
  model,
  tagTools,
...
export async function generateModalityTags(
  modality,
  availableTags,
  model,
  tagTools,
...
export async function generatePreferenceTags(
  preference,
  availableTags,
  model,
  tagTools,
