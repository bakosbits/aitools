 ...
 */
const validateDataForEntity = (entityType, propertyType, rawData, availableItems) => {
    if (!rawData || !rawData[propertyType]) {
        console.error(`No valid ${propertyType}s generated for ${entityType}. AI returned:`, rawData);
        return [];
    }

    const lowerCaseAvailableNames = availableItems.map(item => item.Name.toLowerCase().trim());
    const validatedData = [...new Set(rawData[propertyType]
      .map(name => {
        const lowerCaseName = name.toLowerCase().trim();
        const index = lowerCaseAvailableNames.indexOf(lowerCaseName);
        if (index !== -1) {
...
const validateTagsForEntity = async (
    ...
    try {
        ...
        if (validTagsNames.length !== result.Tags?.length) {
            console.warn(`Some invalid tags were filtered out for ${entityType}. Original:`, result.Tags, "Valid:", validTagsNames);
        }
...
export async function generateToolResearch(tool, model, availableCategories, availableTags) {
    ...
    const validatedCategories = validateDataForEntity('tool', 'Categories', rawResult, availableCategories);
    const validatedTags = validateDataForEntity('tool', 'Tags', rawResult, availableTags);
...
export async function generateUseCaseTags(useCase, availableTags, model, tagTools) {
    const effectiveModel = model || "google/gemini-2.5-flash";
    ...
    const prompt = createTagPrompt('use case',useCase, availableTags, tagTools);
    return validateTagsForEntity('use case', prompt, researchTool, effectiveModel, availableTags);
}

export async function generateModalityTags(modality, availableTags, model, tagTools) {
    const effectiveModel = model || "google/gemini-2.5-flash";
    const schema = getGenericTagsSchema();
    const researchTool = createOpenAITool(
        schema,
    ...
    console.log('Generating modality tags for:', modality, 'with available tags:', availableTags);
...
export async function generatePreferenceTags(preference, availableTags, model, tagTools) {
    const effectiveModel = model || "google/gemini-2.5-flash";
    const schema = getGenericTagsSchema();
    const researchTool = createOpenAITool(
        schema,
...
export async function generateCategoryTags(category, availableTags, model, tagTools) {
    const effectiveModel = model || "google/gemini-2.5-flash";
    const schema = getGenericTagsSchema();
    const researchTool = createOpenAITool(
        schema,
