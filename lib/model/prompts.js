...
2. You MUST Only select tags from list of Available Tags.
3. You MUST NOT create your own tags.
4. If a tool end up having 2 tags that are similar (e.g. generate marketing copy, create social media posts), you must only choose 1 and realign the tag choices for the tool.
...
You MUST ONLY select a maximum of 2 categories for each tool. 
...
3. You MUST select between 3 and 10 tags
...
Based on your analysis, list the top 5 use cases for this tool.
...
3. Each use case should be a short sentence. Include a brief explanation of how the tool addresses the use case.
4. Each use case should be unique.
--- END STRICT REQUIREMENTS ---

--- Use Case Examples ---

1. Generating High-Converting Ad Copy: Anyword excels at creating diverse ad copy variations for platforms like Google, Meta (Facebook/Instagram), and LinkedIn. Its Predictive Performance Score helps marketers identify the most effective headlines and descriptions, minimizing wasted ad spend and boosting conversion rates even before launching campaigns. This also helps overcome writer's block when trying to come up with fresh ad angles.

2. Rapid Prototyping and MVP Development of AI-powered Apps: Bubble is ideal for quickly building and launching Minimum Viable Products (MVPs) for AI-driven ideas. This includes applications that integrate with large language models (LLMs) for natural language processing, image generation APIs, or recommendation engines. Entrepreneurs can test their AI concepts with real users without the need for extensive coding, saving time and resources.

3. Automated Lead Qualification and Generation: Chatfuel excels at engaging website visitors and social media users to qualify leads efficiently. Bots can ask a series of questions, collect contact information, and determine the user's needs and intent, then seamlessly hand off qualified leads to sales teams.

4. Rapid Website Creation from Text Prompts (AI-Powered): Framer's AI capabilities allow users to generate initial website designs, layouts, and even content directly from text descriptions. This significantly accelerates the ideation and initial build phase, enabling designers and non-designers to quickly bring their website concepts to life.

5. Personalizing and Optimizing Email Marketing Campaigns: From subject lines to email body copy, Anyword helps marketers create effective email campaigns. It can generate variations designed to improve open rates and click-through rates, and ensure brand consistency across all email communications. This allows for more engaging and conversion-focused email marketing efforts without the need for extensive manual optimization.
...
export const createUseCaseTagsPrompt = (tool) => {
    const prompt = `You are an expert AI tool analyst. Your task is to identify 1-3 single sentence use cases for the tool named "${tool.Name}".
Based on your analysis, list the use cases for this tool.

--- Example Context ---

Tool: Pipedream
From “Why”:
    Gives developers powerful automation with real code and APIs 
        → use case (sub Pillar): Automate backend workflows using custom code
From “Details”:
    Connects APIs, apps, and custom code in workflows triggered by events 
        → use case (sub Pillar): Build event-driven integrations across services
From “Features”:
    Event Triggers...
    Code Steps...
    Secrets Manager...
        → use case (sub Pillar): Securely manage and trigger API workflows

--- End Example Context ---

For the 'subPillars' field, you MUST follow these STRICT REQUIREMENTS:
1. You MUST provide a list of 1 to 3 use cases.
2. Each use case should be a single sentence.
3. Each use case should be unique.
4. Each use case should not reference the tool name.
--- END STRICT REQUIREMENTS ---
...
Then, call the format_use_case_tags function with the list of use cases.`;
...
export const generateCompareAnalysis = (toolA, toolB, toolAItems, toolBItems, type) => {
    const prompt = `You are an expert analyst comparing two AI tools.
Your audience is trying to decide which tool to use for their specific needs.
Your tone should be objective, insightful, and helpful.

You are comparing ${toolA.Slug} and ${toolB.Slug} based on their ${type}.

Here are the ${type} for ${toolA.Slug}:
${toolAItems.map(item => `- ${item.name}`).join('\n')}

Here are the ${type} for ${toolB.Slug}:
${toolBItems.map(item => `- ${item.name}`).join('\n')}

Please provide a concise analysis and summary of the comparison.
Highlight the key differences and similarities.
Explain which tool might be better for which kind of user or project.
Keep your analysis to 2-3 paragraphs.
