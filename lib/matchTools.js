...
const checkAndScoreCategory = ({
  ...
  const isMatch = selectedItems.every((item) => {
    ...
    return aliases.some((alias) =>
      fieldsToSearch.some((field) => includesIgnoreCase(tool[field], alias)),
    );
  });
...
export function matchTools(tools, useCaseState, allAliases) {
  ...
  return tools
    .map((tool) => {
      ...
      score +=
        useCaseResult.score + modalityResult.score + preferenceResult.score;

      let matchedContext = true;
      if (contexts.length) {
        // Check if contexts filters are active
        matchedContext = contexts.every((cx) => {
          const aliases = CONTEXT_ALIASES[cx]
            ? [cx, ...CONTEXT_ALIASES[cx]]
            : [cx];
          ...
          const hasContextMatch = tool.Categories?.some((category) =>
            aliases.some((alias) =>
              normalize(category.Name).includes(normalize(alias)),
            ),
          );
