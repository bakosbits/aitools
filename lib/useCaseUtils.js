...
export function getInitialState() {
    console.log("[getInitialState] Function called.");
    ...
    try {
        ...
        if (savedState) {
            ...
            if (
                ...
                console.log("[getInitialState] Found valid state in sessionStorage.");
                return parsedState;
            }
        }
    } catch (error) {
        console.error("[getInitialState] Error reading from sessionStorage:", error);
        // Fallthrough to return default state
    }

    console.log("[getInitialState] No valid state in sessionStorage, returning default.");
    return initialUseCaseState;
}
...
export function groupAliasesByType(aliasesArray) {
    ...
    aliasesArray.forEach(alias => {
        ...
        const aliasNames = alias.Aliases ? alias.Aliases.split(',').map(s => s.trim()) : [];
