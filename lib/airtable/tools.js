...
export async function getToolSummaries() {
    try {
        const allToolsRecords = await toolsTable
            .select({
                fields: ["Slug", "Name", "Why", "Description", "Details", "Active"],
        ...
        console.error(
            "[getToolSummaries] ERROR fetching all tools:",
            error.message,
            error.stack,
        );
        throw error;
...
export async function createTool(toolData) {
    try {
        // Remove id property from toolData if present
        const { id, ...fields } = toolData;
        const createdRecords = await toolsTable.create([{ fields }], {
            typecast: true,
        });
