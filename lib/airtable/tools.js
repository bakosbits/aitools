...
export async function getToolsByCategory(Slug) {
  try {
    ...
    if (!category) {
      console.warn(
        `[getToolsByCategory] Category with Slug "${Slug}" not found. Returning empty array.`,
      );
      return [];
    }
    ...
    console.error(
      `[getToolsByCategory] ERROR fetching tools for category "${Slug}":`,
      error.message,
      error.stack,
    );
    throw error;
...
export async function getAllToolsForAdmin() {
  try {
    ...
    console.error(
      "[getAllToolsForAdmin] ERROR fetching all tools for admin:",
      error,
    );
    throw error;
  }
