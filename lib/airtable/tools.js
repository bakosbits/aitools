import { shuffleArray } from "../indexUtils";
import { toolsTable, categoriesTable } from "@/lib/airtable/base";

export async function getToolsForTagUpdates() {
    try {
...
const mapToolRecord = (record) => {
    ...
    const cautions = record.get("Cautions") || "";
...
export async function updateTool(recordId, toolData) {
    try {
        console.log(
            "[updateTool] Updating tool with data:",
...
export async function updateFeatureTags(toolId, featureTagIds) {
    try {
        const updatedRecord = await toolsTable.update([
            {
                id: toolId,
                fields: {
                    FeatureTags: featureTagIds,
                },
            },
        ]);
        return mapToolRecord(updatedRecord[0]);
    } catch (error) {
        console.error(`Error updating tool ${toolId} with tags:`, error);
        throw error;
    }
}

export async function updateCautionTags(toolId, cautionTagIds) {
    try {
        const updatedRecord = await toolsTable.update([
            {
                id: toolId,
...
export async function updateUseCaseTags(toolId, useCaseTagIds) {
    try {
        const updatedRecord = await toolsTable.update([
            {
                id: toolId,
        ...
        console.error(
            `Error updating tool ${toolId} with use case tags:`,
            error,
        );
        throw error;
    }
...
export async function getTagTools(count = 30) {
    try {
        const toolRecords = await toolsTable
            .select({
                filterByFormula: "{Active} = TRUE()",
        ...
        const shuffledTools = shuffleArray(toolRecords);
        const mappedTools = shuffledTools.map((record) => ({
            Name: record.fields["Name"],
            Description: record.fields["Description"] || null,
            Why: record.fields["Why"] || null,
