...
export async function updateManyFeatures(records) {
    try {
        const updatedRecords = await toolFeaturesTable.update(
            records.map((r) => ({ id: r.id, fields: r })),
        );
        return updatedRecords.map((record) => mapRecord(record, "feature"));
    } catch (error) {
