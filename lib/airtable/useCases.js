...
export async function updateManyUseCases(records) {
    try {
        const updatedRecords = await toolUseCasesTable.update(
            records.map((r) => ({ id: r.id, fields: r })),
        );
        return updatedRecords.map((record) => mapRecord(record, "useCase"));
    } catch (error) {
        console.error("[updateMany] ERROR updating use cases:", error);
        throw error;
    }
}
