...
const getBaseSchema = (categoryNames) => ({
  ...
  Tags: {
    ...
    items: { type: "STRING" },
    description:
      "An array of seperate strings, with each string being a tag for a tag cloud (3 to 7 cloud tags total).",
  },
  Categories: {
    type: "ARRAY",
    items: {
...
export async function generateToolResearch(toolName, modelName, categoryNames) {
  ...
  const prompt = `You are an expert AI tool researcher. Research the tool named "${toolName}". 
    Based on your research, identify the most relevant professional categories for this tool. 
    Available Categories: ${categoryNames.join(", ")}. It is critical that all 'Required' fields in 
    the schema are filled accurately and not left empty. Then, call the 'format_tool_research' function with the results.`;
...
Write a comprehensive and persuasive affiliate blog post about "${topic}". The blog post should be between 2500 and 3000 words.
