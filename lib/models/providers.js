...
import {
    ...
    createQuickToolPrompts,
    createToolRecommendationPrompt,
...
import {
    ...
    getToolRecommendationSchema,
} from "@/lib/models/schemas";

/**
 * Simple text completion using OpenRouter (or OpenAI-compatible) API.
 * @param {object} options - { prompt: string, model?: string }
 * @returns {Promise<string>} - The generated text content.
 */
export async function createWithOpenAI({ prompt, model }) {
...
export const createOpenAITool = (schema, functionName, functionDescription) => {
    const properties = {};
    const required = [];
    const typeMap = { STRING: "string", ARRAY: "array", OBJECT: "object" };
    ...
    for (const [key, value] of Object.entries(schema)) {
        ...
        if (value.items) {
            ...
            if (value.items.properties) {
                properties[key].items.properties = value.items.properties;
            }
        }
        if (value.enum) {
...
/**
 * Generates quick content for a given toolType and inputText using the appropriate prompt and model.
 * @param {string} toolType - The type of quick tool (e.g., 'headline', 'tweet', etc.)
 * @param {string} inputText - The user input text.
 * @param {string} [model] - Optional model override.
 * @returns {Promise<string>} - The generated content string.
 */
export async function generateQuickContent(toolType, inputText, model) {
    const promptFn = createQuickToolPrompts[toolType] || quickToolPrompts.default;
    const prompt = promptFn(inputText);
    return createWithOpenAI({
        prompt,
        model: model || "google/gemini-2.5-flash",
    });
}
...
export async function generateToolRecommendation(userGoals, experienceLevel, priorities, requiredFeatures, filteredTools, model) {
    const effectiveModel = model || "google/gemini-2.5-flash";
    const schema = getToolRecommendationSchema();

    const recommendationTool = createOpenAITool(
        schema,
        "format_tool_recommendation",
        "Formats the tool recommendation based on user needs.",
    );

    const prompt = createToolRecommendationPrompt(userGoals, experienceLevel, priorities, requiredFeatures, filteredTools);
    const result = await generateDataForEntity(
        prompt,
        recommendationTool,
        effectiveModel,
