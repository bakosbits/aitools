...
import {
    mapTagPrompt, mapCategoriesPrompt, createToolResearchPrompt,
    createUseCasesPrompt, createCautionsPrompt, createFeaturesPrompt, createUseCaseTagsPrompt,
} from "@/lib/modelss/prompts";
import {
    getBaseSchema, getToolCategoriesSchema, getGenericTagsSchema,
    getUseCasesSchema, getCautionsSchema, getFeaturesSchema, getUseCaseTagsSchema,
} from "@/lib/modelss/schemas";
...
const validateTagsForEntity = async (
    ...
    try {
        ...
        const isStringArray = availableTags.length > 0 && typeof availableTags[0] === 'string';
...
export async function mapToolCategories(tool, availableCategories, toolFeatures, model) {
    const effectiveModel = model || "google/gemini-2.5-flash";
...
export async function mapCautionTags(tool, availableTags, model, cautions) {
    ...
    return validateTagsForEntity("cautions", prompt, researchTool, effectiveModel, availableTags);
}
...
export async function generateUseCases(tool, model) {
    ...
    const result = await generateDataForEntity(prompt, useCasesTool, effectiveModel);
...
export async function generateCautions(tool, model) {
    ...
    const result = await generateDataForEntity(prompt, researchTool, effectiveModel);
