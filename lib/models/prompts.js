// Quick tool prompt templates
export const createQuickToolPrompts = {
    headline: input => `Rewrite the following headline into five different versions with varied tones: ${input}`,
    tweet: input => `Expand the following idea into a 5–8 tweet Twitter thread: ${input}`,
    tagline: input => `Create five catchy taglines for this idea: ${input}`,
    image: input => `Generate three detailed descriptions for an image based on the following input:\n\n${input}`,
    blog: input => `Create a detailed blog post outline for the following topic:\n\n${input}`,
    quote: input => `Generate 3 meaningful quotes from the following input:\n\n${input}`,
    default: input => `Respond to: ${input}`,
};
...
export const createToolRecommendationPrompt = (userGoals, experienceLevel, priorities, requiredFeatures, filteredTools) => {
    return `You are an expert AI tool advisor. Your task is to analyze the user's needs and recommend the most suitable AI tool from the available options.
...
Based on the user's profile and the available tools, select the SINGLE BEST TOOL that matches their needs. Consider:
1. How well the tool's capabilities align with the user's goals
2. Whether the tool's complexity matches their experience level
3. If the tool satisfies their key priorities
4. Whether it provides the required features
5. Any potential limitations or considerations they should be aware of

Then, call the 'format_tool_recommendation' function with:
1. The name of the recommended tool
2. 3-4 specific reasons why it's the best match
3. 1-2 potential limitations they should be aware of
4. 1-2 alternative tools that could also meet their needs (with key benefits)

Your recommendation should be clear, specific, and focused on the user's actual needs.`;
};